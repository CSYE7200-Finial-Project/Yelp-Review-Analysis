...
class PreDataML3 {
  ...
  def getRows(seqSet: Seq[RDD[String]], sourceFile: Seq[RDD[String]], sparkC: SparkContext) = {
    val writer = new PrintWriter(new File("src/main/resources/wordFreLine/wordFreLine.txt"))
    val rr = for { i <- 0 to 4 } yield {
      ...
      } yield writer.write(percents(0) + "\t" + percents(1) + "\t" + percents(2) + "\t" + percents(3) + "\t" + percents(4) + "\t" + (i+1) + "\n")
      rows
    }
    writer.close()
  }
